...
const TaskDetails: React.FC = () => {
  ...
  const fetchTaskDetails: any = async () => {
    const res = await axios.get(
      `http://46.101.172.171:8008/tasks/item/${task_id}`,
      axiosConfig
    );
    return res.data;
  };

  const { error, data: taskInfo } = useQuery<any, any>(
    `details-for-task-${task_id}`,
    fetchTaskDetails
  );
  ...
  const {
    ...
    fetchMore,
    canFetchMore,
  } = useInfiniteQuery<CommentType, [any, any], any>(
    [`comments`, task_id],
  ...
  const addComment = async ({ task_id, text }: any) => {
    ...
    const res = await axios.post(
      `http://46.101.172.171:8008/comment/to_task/${task_id}`,
      { text },
      axiosConfig
    );
    return res.data;
  ...
  const [addCommentMutate] = useMutate(
    addComment,
    [`comments`, task_id],
    'TaskDetailsAddComment',
    false,
    'none'
  );
  ...
  const assignTagToTask = async (id: number) => {
    const res = await axios.get(
      `http://46.101.172.171:8008/tags/task_tag/set/${task_id}/${id}`,
      axiosConfig
    );
    return id;
  };
    ...
    <div className='details-container'>
          ...
          disabled={
            ...
            (comments &&
              comments[comments.length - 1].page_current >=
                comments[comments.length - 1].page_total)
          }
          className={
      ...
      <div className='add-section comments-add'>
        ...
        <textarea
          ...
          cols={80}
          ref={commentArea}
          className='description-area details-input'
        />
        <button
          className='btn'
          onClick={() =>
            addCommentMutate({ task_id, text: commentArea.current!.value })
          }
        >
          Add Comment
        </button>
      ...
      <div className='add-section tags-add'>
        ...
        <button
          className='btn'
          onClick={() => addTagMutate(tagArea.current!.value)}
        >
          Add Tag
        </button>
        {tagExist && (
          <div className='tag-exists'>Sorry, this tag already exists</div>
        )}
      </div>
    </div>
  );
