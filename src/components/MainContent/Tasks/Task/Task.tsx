...
const Task: React.FC<ITask> = ({
  ...
  const showHideDescription = () => {
    let setTo = showDescription ? false : true;
    ...
    if (parent) {
      ...
      await Promise.all(
        parent.map((subtask: any) =>
          axios
            .get(
              `http://46.101.172.171:8008/tasks/item/${subtask.child}`,
              axiosConfig
            )
            .then((res: any) => {
    ...
    <div className="task-row">
      <span onClick={completeTask}>
        <Icon
          icon={checkCircle}
          className={
            'check icon ' + (isCompleted ? 'task-completed-check' : '')
          }
        />
      </span>
      <span className="assigned-to">
        ...
        <img
          src={assignPencil}
          alt="assign to"
          className="assign-pencil icon"
        />
      </span>
      <div className="task-info-tooltip">
        <Link
          to={`task_info/${id}`}
          onClick={evt => (task_list === '0' ? evt.preventDefault() : null)}
        >
          <span
            className={'task-title ' + (isCompleted ? 'title-completed' : '')}
          >
      ...
      {description ? (
        <span className="open-close-description" onClick={showHideDescription}>
      ...
      <Icon
        icon={alertCircleCheck}
        className="exclamation important icon pd-left-10"
      />
      ...
      {task_list && (
        <div className="description-tooltip">
          <img
            src={comments}
            alt="add comment"
            className="comments icon pd-left-10"
          />
