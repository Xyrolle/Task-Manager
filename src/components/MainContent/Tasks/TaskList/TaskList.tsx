...
const TaskList = ({ name, id, task_count, description }: any) => {
  ...
  const deleteTaskList: any = ({ id }: any) => {
    axios
      .delete(
        `http://46.101.172.171:8008/project/tasklist_delete/${id}`,
        axiosConfig
      )
      .catch(() => console.log('Error: can not delete a task list'));
  };

  const [deleteTaskListMutate] = useMutate(
    deleteTaskList,
    'task-lists',
    'TaskListDelete',
    false,
    ...
    <Fragment>
      <div className='task-list-row'>
        <img
          src={arrow}
          alt='arrow'
          className={'show-tasks-arrow ' + (isOpen ? 'arrow-close' : '')}
          onClick={() => {
          ...
          <div className='taskList-content'>
            <ul>
              <li
                ...
                onClick={() => deleteTaskListMutate({ id })}
              >
                Delete Task List
              </li>
              <li
      ...
      {tasks.data && (
        <div className='task-container'>
          ...
          {isOpen && !tasks.data.length && (
            <div className='no-tasks'>No tasks in this list yet.</div>
          )}
          {isOpen &&
            tasks.data.length > 0 &&
...
const fetchTasks = async (id: any) => {
  try {
    const res = await axios.get(
      `http://46.101.172.171:8008/tasks/task_list/${id}/1`,
      {
        headers: {
          Authorization: `Basic YWRtaW46cXdlMTIz`,
        },
