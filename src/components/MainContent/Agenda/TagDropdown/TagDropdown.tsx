...
const setTagToAgenda = async (agendaId: number, id: string) => {
  const response = await axios.get(`http://46.101.172.171:8008/tags/agenda_tag/set/${agendaId}/${id}`,
    axiosConfig)
  return response.data
}

const createTag = async ({ title, agendaId, projectId }: createTagInterface): Promise<void> => {
